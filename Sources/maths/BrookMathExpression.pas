...
type
  ...
  TBrookMathExpressionError = {$ifdef DELPHI}packed record{$else}class{$endif}
  private
    FHandle: Psg_expr;
  ...
  TBrookMathExpressionExtension = {$ifdef DELPHI}packed record{$else}class{$endif}
  private
  ...
  private
    ...
    FCompiled: Boolean;
    procedure DoExtensionsChange(aSender: TObject);
...
begin
  ...
  TSgLib.UnloadEvents.Add(InternalLibUnloadEvent, Self);
end;
...
begin
  SetActive(False);
  FExtensions.Free;
  TSgLib.UnloadEvents.Remove(InternalLibUnloadEvent);
  inherited Destroy;
end;
...
procedure TBrookMathExpression.DoExtensionsChange(aSender: TObject);
begin
  Clear;
end;
...
begin
  if not Assigned(FHandle) then
    Exit;
  TSgLib.Check;
  TSgLib.CheckLastError(sg_expr_clear(FHandle));
{$IFNDEF FPC}
...
begin
  CheckActive;
  TSgLib.CheckLastError(sg_expr_set_var(FHandle, M.ToCString(AName),

...
begin
  ...
  try
    ...
    begin
      VHandled:= True;
      BrookSettingsInstance.OnError(VRes, AException, VHandled);
      if VHandled then
        Exit;
    end;
...
begin
  if BrookSettingsInstance.Port <> 0 then
    Port := BrookSettingsInstance.Port;
  TRequest.DefaultRequestUploadDir := BrookSettingsInstance.DirectoryForUploads;
  if TRequest.DefaultRequestUploadDir = '' then
    TRequest.DefaultRequestUploadDir := TSagui.TmpDir;

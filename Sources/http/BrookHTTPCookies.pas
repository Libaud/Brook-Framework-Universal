...
class function TBrookHTTPCookie.Sign(const ASecret, AUnsignedValue: string): string;
var
{$IFDEF FPC}
  VEncoder: TBase64EncodingStream;
  VStream: TStringStream;
  VDigest: THMACSHA1Digest; { Todo -o Frédéric : not used }
{$ELSE}
...
begin
  ...
  begin
    ...
    if (Length(AUnsignedValue) > 0) and
      (CompareStr(TBrookTools.Sha1(Sign(ASecret, AUnsignedValue)),
        TBrookTools.Sha1(ASignedValue)) = 0) then
      Exit(True);
  end;
  Result := False;
end;
...
begin
  ...
  if FExpires > -1 then
    Result := Concat(Result, '; Expires=', TBrookTools.DateTimeToGmt(FExpires));
  if FHttpOnly then
...
begin
  ...
  FMaxAge := AValue;
  if AValue > 0 then
    FExpires := TBrookTools.DateTimeToUTC(IncSecond(Now, AValue))
  else
    Expire;
end;
...
begin
  if AValue <> FPath then
    FPath := TBrookTools.FixPath(AValue);
end;

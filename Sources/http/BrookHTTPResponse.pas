...
type
  ...
  protected
    class function DoStreamRead(Acls: Pcvoid; Aoffset: cuint64_t; Abuf: Pcchar; Asize: csize_t): cssize_t; cdecl; static;
    class procedure DoStreamFree(Acls: Pcvoid); cdecl; static;
    class procedure CheckStatus(AStatus: Word); static; {$IFNDEF DEBUG}inline;{$ENDIF}
    class procedure CheckStream(AStream: TStream); static; {$IFNDEF DEBUG}inline;{$ENDIF}
...
uses
  SAGuiGlobal, SAGuiClasses, SAGuiInternalUtilities;
...
class function TBrookHTTPResponse.DoStreamRead(Acls: Pcvoid; Aoffset: cuint64_t; //FI:O804
                                               Abuf: Pcchar; Asize: csize_t): cssize_t; cdecl;
begin
...
begin
  TSgLib.Check;
  TSgLib.CheckLastError(sg_httpres_set_cookie(FHandle, M.ToCString(AName),
    M.ToCString(AValue)));
end;
...
begin
  CheckStream(AStream);
  CheckStatus(AStatus);
  if AFreed and (not TSgLib.IsLoaded) then
    AStream.Free;
  TSgLib.Check;
  if AFreed then
    FCb := {$ifdef FPC}@{$endif}DoStreamFree
  else
    FCb := nil;
  if FCompressed then
  begin
    R := sg_httpres_zsendstream(FHandle, {$ifdef FPC}@{$endif}DoStreamRead, AStream, FCb, AStatus);
    CheckZLib(R);
  end
  else
    R := sg_httpres_sendstream(FHandle, 0, {$ifdef FPC}@{$endif}DoStreamRead, AStream, FCb, AStatus);
  CheckAlreadySent(R);
  TSgLib.CheckLastError(R);
end;
...
begin
  TSgLib.Check;
  TSgLib.CheckLastError(sg_httpres_reset(FHandle));
end;

procedure TBrookHTTPResponse.Clear;
begin
  TSgLib.Check;
  TSgLib.CheckLastError(sg_httpres_clear(FHandle));
end;

...
type
  ...
  { Structured type which holds the upload properties and allows to save the
    uploaded file. }
  TBrookHTTPUpload = {$ifdef DELPHI}packed record{$else}class{$endif}
  private
...
uses
  SAGuiClasses, SAGuiGlobal, SAGuiWrapper;
...
begin
  TSgLib.Check;
  TSgLib.CheckLastError(sg_httpupld_save(FHandle, AOverwritten));
end;
...
begin
  TSgLib.Check;
  TSgLib.CheckLastError(sg_httpupld_save_as(FHandle, M.ToCString(APath),
    AOverwritten));
end;
...
begin
  TSgLib.Check;
  TSgLib.CheckLastError(sg_httpuplds_next(@FCurrent));

...
begin
  if FAuthenticated then
    VACb := {$ifdef FPC}@{$endif}DoAuthenticationCallback
  else
    VACb := nil;
  FHandle := sg_httpsrv_new2(VACb, {$ifdef FPC}@{$endif}DoRequestCallback, {$ifdef FPC}@{$endif}DoErrorCallback, Self);
...
class function TBrookHTTPServer.DoAuthenticationCallback(Acls: Pcvoid; Aauth: Psg_httpauth; Areq: Psg_httpreq; Ares: Psg_httpres): cbool; cdecl;
var
...
begin
  ...
  InternalCheckServerOption(sg_httpsrv_set_cli_cb(FHandle,
    {$ifdef FPC}@{$endif}DoClientConnectionCallback, Self));

...
{$ENDIF}
  ...
  private
    ...
    function GetPaths: {{$ifdef FPC}specialize{$endif} TArray<string>} TStringArray; {$IFNDEF DEBUG}inline;{$ENDIF}
  ...
  protected
    class procedure DoRequestIsolatedProcCallback(Acls: Pcvoid; Areq: Psg_httpreq; Ares: Psg_httpres); cdecl; static;
{$IFNDEF FPC}
...
{$ENDIF}
    ...
    property Paths: {$ifdef FPC}specialize{$endif}TArray<string> read GetPaths;
...
type
  ...
  {$ifdef FPC}generic{$endif} TBrookHTTPReqIsolatedProcHolder<T> = class
  private
    FProc: T;
    FUserData: Pointer;
  public
...
constructor TBrookHTTPReqIsolatedProcHolder{$ifndef FPC}<T>{$endif}.Create(const AProc: T; AUserData: Pointer);
begin
  inherited Create;
  FProc := AProc;
  FUserData := AUserData;
...
class procedure TBrookHTTPRequest.DoRequestIsolatedProcCallback(Acls: Pcvoid; Areq: Psg_httpreq; Ares: Psg_httpres); cdecl;
var
  VHolder: {$ifdef FPC}specialize{$endif} TBrookHTTPReqIsolatedProcHolder<TBrookHTTPRequestIsolatedProc>;
  VReq: TBrookHTTPRequest;
  VRes: TBrookHTTPResponse;
begin
  {VHolder := Acls;} { Todo -o Frédéric : implementation pb }
  try
...
begin
  ...
  if not Assigned(C) then
    Exit('');
  TSgLib.CheckLastError(sg_ip(C, @P[0], SizeOf(P)));
  Result := TMarshal.ToString(@P[0]);
end;

function TBrookHTTPRequest.GetPaths: {TArray<string>} TStringArray;
begin
...
type
  TBrookHTTPQueryIsolated = specialize TBrookHTTPReqIsolatedProcHolder<TBrookHTTPRequestIsolatedProc>;

procedure TBrookHTTPRequest.Isolate(AProc: TBrookHTTPRequestIsolatedProc; AUserData: Pointer);
var
  //VHolder: TBrookHTTPReqIsolatedProcHolder<TBrookHTTPRequestIsolatedProc>;
  VHolder: TBrookHTTPQueryIsolated;
begin
  TSgLib.Check;
  SetIsIsolated(True);
  {VHolder := TBrookHTTPReqIsolatedProcHolder<
    TBrookHTTPRequestIsolatedProc>.Create(AProc, AUserData);}
  VHolder := TBrookHTTPQueryIsolated.Create(AProc, AUserData);
  try
    TSgLib.CheckLastError(sg_httpreq_isolate(FHandle,
      {$ifdef FPC}@{$endif}DoRequestIsolatedProcCallback, VHolder));
  except
    VHolder.Free;
    raise;
  end;
...
begin
  TSgLib.Check;
  TSgLib.CheckLastError(sg_httpreq_set_user_data(FHandle, AValue));
end;

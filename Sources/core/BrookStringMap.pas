...
uses
  Classes, SysUtils, RTLConsts, StrUtils, TypInfo, Platform, Marshalling,
  BrookUtility, BrookHandledClasses, LibSAGuiAPITypes;
...
type
  ...
  TBrookStringPair = {$ifdef DELPHI}packed record{$else}class{$endif}
  private
    FName: string;
    FValue: string;
  public
  ...
  protected
    class function CreatePair(Apair: Psg_strmap): TBrookStringPair; static; {$IFNDEF DEBUG}inline;{$ENDIF}
    class function DoIterate(aCls: Pcvoid; aPair: Psg_strmap): cint; cdecl; static;
    class function DoSort(Acls: Pcvoid; Apair_a: Psg_strmap; Apair_b: Psg_strmap): cint; cdecl; static;
...
class function TBrookStringMap.DoIterate(aCls: Pcvoid; aPair: Psg_strmap): cint; cdecl;
var
  _Method: PMethod;
begin
  _Method := aCls;
  if not Assigned(_Method{$ifdef FPC}^{$endif}.Code) then
    Exit(-1);
  Result := TBrookStringMapIterator(_Method{$ifdef FPC}^{$endif}.Code)(_Method{$ifdef FPC}^{$endif}.Data, CreatePair(aPair));
end;

class function TBrookStringMap.DoSort(Acls: Pcvoid; Apair_a: Psg_strmap; Apair_b: Psg_strmap): cint; cdecl;
var
  _Method: PMethod;
begin
  _Method := Acls;
  if not Assigned(_Method{$ifdef FPC}^{$endif}.Code) then
    Exit(0);
  Result := TBrookStringMapComparator(_Method{$ifdef FPC}^{$endif}.Code)(_Method{$ifdef FPC}^{$endif}.Data,
                                      CreatePair(Apair_a),
                                      CreatePair(Apair_b));
end;
...
var
  _Marshaller: TMarshaller;
begin
  TSgLib.Check;
  TSgLib.CheckLastError(sg_strmap_add(FHandle,
                                      _Marshaller.ToCString(AName),
                                      _Marshaller.ToCString(AValue)));
  DoChange(sgmoAdd);
end;
...
begin
  TSgLib.Check;
  TSgLib.CheckLastError(sg_strmap_set(FHandle,
                                      _Marshaller.ToCString(AName),
                                      _Marshaller.ToCString(AValue)));
...
begin
  TSgLib.Check;
  TSgLib.CheckLastError(sg_strmap_next(@FNextHandle));
  Result := Assigned(FNextHandle);
  if Result then
    APair := CreatePair(FNextHandle);
end;
...
begin
  ...
  M.Code := @AIterator;
  M.Data := AData;
  R := sg_strmap_iter(FHandle^, {$ifdef FPC}@{$endif}DoIterate, @M);
  if R <> -1 then
    TSgLib.CheckLastError(R);
end;
...
begin
  ...
  M.Code := @AComparator;
  M.Data := AData;
  TSgLib.CheckLastError(sg_strmap_sort(FHandle, {$ifdef FPC}@{$endif}DoSort, @M));
end;

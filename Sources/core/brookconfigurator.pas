...
begin
  ...
  try
    ...
    if (not Assigned(FTarget)) or VHandled then
      Exit;
    if (FParams.Count = 0) and (BrookSettingsInstance.Configuration <> ES) then
    begin
      ...
      try
        ...
        if (Pos(SC, BrookSettingsInstance.Configuration) <> 0) or
          (Pos(EQ, BrookSettingsInstance.Configuration) <> 0) then
          FParams.DelimitedText := BrookSettingsInstance.Configuration
        else
        begin
          if not FileExists(BrookSettingsInstance.Configuration) then
            raise EBrookConfigurator.CreateFmt(Self,
              SBrookCfgFileNotFoundError, [BrookSettingsInstance.Configuration]);
          FParams.LoadFromFile(BrookSettingsInstance.Configuration);
        end;
      finally

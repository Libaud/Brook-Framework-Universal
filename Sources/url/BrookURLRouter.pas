...
type
  ...
  private
    ...
    function GetSegments: TStringArray;
  protected
    class procedure DoRouteCallback(Acls: Pcvoid; Aroute: Psg_route); cdecl; static;
    class function DoSegmentsIterCallback(Acls: Pcvoid; Aindex: cuint; const Asegment: Pcchar): cint; cdecl; static;
    class function DoVarsIterCallback(Acls: Pcvoid; const Aname: Pcchar; const Aval: Pcchar): cint; cdecl; static;
  ...
  public
    ...
    property Segments: TStringArray read GetSegments;
...
uses
  LibSAGuiAPIConsts, SAGuiclasses, SAGuiGlobal, SAGuiWrapper, BrookTools, BrookHTTPRequestHelper;
...
class procedure TBrookURLRoute.DoRouteCallback(Acls: Pcvoid; Aroute: Psg_route); cdecl;
var
  VRoute: TBrookURLRoute;
begin
  VRoute := TBrookURLRoute(Acls);
...
class function TBrookURLRoute.DoSegmentsIterCallback(Acls: Pcvoid; Aindex: cuint; //FI:O804
                                                     const Asegment: Pcchar): cint; cdecl;
var
  VSegments: TStringArray;
begin
  VSegments:= Acls;
...
class function TBrookURLRoute.DoVarsIterCallback(Acls: Pcvoid; const Aname: Pcchar; const Aval: Pcchar): cint; cdecl;
begin
...
function TBrookURLRoute.GetSegments: TStringArray;
begin
  Result := nil;
  if not Assigned(FHandle) then
    Exit(nil);
  TSgLib.Check;
  TSgLib.CheckLastError(sg_route_segments_iter(FHandle, {$ifdef FPC}@{$endif}DoSegmentsIterCallback, @Result));
end;
...
begin
  ...
  FVariables.Clear;
  TSgLib.Check;
  TSgLib.CheckLastError(sg_route_vars_iter(FHandle, {$ifdef FPC}@{$endif}DoVarsIterCallback, FVariables));
end;
...
begin
  ...
  R := sg_routes_add2(@FHandle, @H, M.ToCNullableString(GetRoutePattern(ARoute)),
                      @P[0], SG_ERR_SIZE, {$ifdef FPC}@{$endif}(ARoute.DoRouteCallback), ARoute);
  if R = 0 then
    Exit;
  if R = EALREADY then
...
begin
  if Assigned(FHandle) or (Count = 0) then
    Exit;
  TSgLib.Check;
  TSgLib.CheckLastError(sg_routes_cleanup(@FHandle));
  for RT in Self do
  begin
    RT.Validate;
    InternalAdd(RT);
...
begin
  ...
  begin
    if Assigned(FHandle) then
      TSgLib.CheckLastError(sg_routes_rm(@FHandle, M.ToCString(APattern)));
    inherited Delete(I);
  end;
end;
...
begin
  ...
  CheckActive;
  TSgLib.Check;
  R := sg_router_dispatch(FHandle, M.ToCNullableString(TBrookTools.FixPath(APath)), AUserData);
  Result := R = 0;
  if (not Result) and (R <> ENOENT) then
    TSgLib.CheckLastError(R);
end;

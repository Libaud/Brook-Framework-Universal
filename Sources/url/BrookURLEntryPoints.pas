...
uses
  SAGuiClasses, SAGuiGlobal, BrookTools;
...
begin
  Result := TBrookURLRouter(GetUserData);
end;
...
begin
  if (AValue = FName) or (not Assigned(FList)) then
    Exit;
  NN := TBrookTools.FixEntryPoint(AValue);
...
begin
  ...
  if sg_entrypoints_find(FList.FHandle, @EP, M.ToCString(FName)) = 0 then
    TSgLib.CheckLastError(sg_entrypoint_set_user_data(EP, FUserData));
end;
...
begin
  ...
  TSgLib.CheckLastError(sg_entrypoints_clear(FHandle));
  for EP in Self do
  begin
    EP.Validate;
    InternalAdd(EP);
...
begin
  ...
  begin
    if Assigned(FHandle) then
      TSgLib.CheckLastError(sg_entrypoints_rm(FHandle, M.ToCString(AName)));
    inherited Delete(I);
  end;
end;
...
begin
  CheckItems;
  CheckActive;
  EP := TBrookTools.FixEntryPoint(APath);
  P := TBrookTools.FixPath(APath.SubString(EP.Length));
